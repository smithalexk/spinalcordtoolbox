...
def panda_dataset(path_local, cc):
    ...
    data_lst = [l.split('_'+cc)[0] for l in data_lst]
    ...
    pd_tmp = pd.DataFrame.from_dict(dct_tmp)
    hc_lst = pd_tmp[pd_tmp.patho=='hc'].subj_name.values.tolist()
    if cc=='t1':
        lambda_rdn = 0.23
    else:
        lambda_rdn = 0.33
    ...
    pd_tmp['valid_test'] = ['test' for i in range(len(data_lst))]
    for s in training_lst:
        pd_tmp.loc[pd_tmp.subj_name==s,'valid_test'] = 'train'

    print '# of patient in: '
    print '... testing:' + str(len(pd_tmp[(pd_tmp.patho == 'patient') & (pd_tmp.valid_test == 'test')]))
    print '... validation:' + str(len(pd_tmp[(pd_tmp.patho == 'patient') & (pd_tmp.valid_test == 'valid')]))
    print '# of no-iso in: '
    print '... testing:' + str(len(pd_tmp[(pd_tmp.resol == 'not') & (pd_tmp.valid_test == 'test')]))
    print '... validation:' + str(len(pd_tmp[(pd_tmp.resol == 'not') & (pd_tmp.valid_test == 'valid')]))

    pd_tmp.to_pickle(path_local + 'test_valid_' + cc + '.pkl')
